(function($) {
    $.extend({
        "foucs": function(con) {
            ...
            silde = function(d) {
                $imgs.eq((d ? getPrevIndex(1) : getNextIndex(1))).css("left", (d ? "-1920px": "1920px"));
                $imgs.animate({
                    "left": (d ? "+": "-") + "=1920px"
                },
                config.animateTime);
            ...
            sildeJump = function(d) {
                if (d >= 6) {
                    ...
                    $imgs.eq(0).css("left", "1920px")
                } else {
                    if (d == 0) {
                        $imgs.eq(0).css("left", "0px");
                        $imgs.eq(1).css("left", "1920px");
                        $imgs.eq(2).css("left", "3840px");
                        $imgs.eq(3).css("left", "5760px");
                        $imgs.eq(4).css("left", "-5760px");
                        $imgs.eq(5).css("left", "-3840px");
                        $imgs.eq(6).css("left", "-1920px")
                    } else {
                        for (var j = d + 1; j >= 0; j--) {
                            var px = (j - d) * 1920;
                            $imgs.eq(j).css("left", px + "px")
                        }
                        for (var k = j = d + 1; j > 0; j--, k++) {
                            var px = (j + 1) * -1920;
            ...
            config.interval);
            $imgs.css("left", "-1920px");
            $imgs.eq(i).css("left", 0).end().eq(i + 1).css("left", "1920px");
